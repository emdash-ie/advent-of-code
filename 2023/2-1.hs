...
main = Text.interact \input -> let
  ...
  totals = fmap (\g@Game{id} -> (id, totalReveals g)) games
  isPossible rs =
    (Map.lookup Red rs <= Just 12)
    && (Map.lookup Green rs <= Just 13)
    && (Map.lookup Blue rs <= Just 14)
  possible = filter (\(_, totalReveals) -> isPossible totalReveals) totals
  total = sum (fmap fst possible)
